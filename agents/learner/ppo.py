...
class PPO(RLAgent):
    ...
    def initialize_optimizer(self):
        ...
        if not isinstance(self.optimizer, torch.optim.Adam):
            raise ValueError(f"Invalid NavA2C optimizer: " 
    ...
    #                                        NavA2C CORE METHODS                                        #
    # ================================================================================================= #
    def initialize_episode(self, scenario: tuple, episode_counter: int):
        ...
        return super().initialize_episode(scenario, episode_counter)
    ...
    def training_iteration(self, episode_counter: int):
        ...
        for log_prob, value, reward in zip(self.episode_log_probabilities, 
            ...
            critic_losses.append(F.smooth_l1_loss(value, torch.tensor([[reward]], dtype=torch.float64, device=Config.DEVICE)))

        total_loss = torch.stack(actor_losses).mean() + torch.stack(critic_losses).mean() - \
                     Config.A2C.ENTROPY_FACTOR * torch.stack(self.episode_entropies).mean()

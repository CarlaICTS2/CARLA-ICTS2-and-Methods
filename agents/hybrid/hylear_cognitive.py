...
class HyLEAR_NavA2C_Cognitive(ISDespotP):

    def __init__(self, client, world, carla_map, scenario):
        # if Config.MODE is Mode.TRAIN:
        super(HyLEAR_NavA2C_Cognitive, self).__init__(client, world, carla_map, scenario)
    ...
    def run_step(self, step_counter: int):
        ...
        obstacles = super(HyLEAR_NavA2C_Cognitive, self).get_obstacles(start)
        ...
        super(HyLEAR_NavA2C_Cognitive, self).get_current_observation(step_counter)

        # need a non-trivial path for IS-DESPOT
        if Config.MODE is Mode.TRAIN:
            birdview_car_intention = super(HyLEAR_NavA2C_Cognitive, self).get_birdview_car_intention(
            ...
            despot_action, despot_value, despot_policy = super(HyLEAR_NavA2C_Cognitive, self).get_speed_action(step_counter)

            # construct categorical distribution
            action_policy = torch.tensor([0.05, 0.5, 0.5], dtype=torch.float64, device="cpu") # cuda not worth it
            ...
            # calculate reward
            step_summary = super(HyLEAR_NavA2C_Cognitive, self).get_reward_akash(step_counter)
        ...
        if not terminated_early and Config.MODE is Mode.TRAIN:
            ...
            if episode_counter != 0 and episode_counter % Config.MODEL_SAVE_FREQUENCY_EPISODES == 0:
                save_model(model_state_dict=self.model.state_dict(),
                        ...
                        name="hyLear_Cognitive_high_sample")

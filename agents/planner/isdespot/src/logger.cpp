...
std::vector<int> EvalLog::tree_nodes;
std::vector<int> EvalLog::backups;
std::vector<int> EvalLog::terminal_states;

std::vector<std::vector<double>> EvalLog::root_lower_bounds;
std::vector<std::vector<double>> EvalLog::root_upper_bounds;

std::vector<std::vector<double>> EvalLog::initial_lower_bounds;
std::vector<std::vector<double>> EvalLog::initial_upper_bounds;

std::vector<std::vector<double>> EvalLog::trial_depths; 
std::vector<std::vector<double>> EvalLog::excess_uncertainties; 
std::vector<std::vector<double>> EvalLog::observations;

// fine-grained execution time tracking
std::vector<double> EvalLog::search_execution_times;
std::vector<double> EvalLog::construct_tree_execution_times;

std::vector<double> EvalLog::trial_execution_times;
std::vector<double> EvalLog::expand_execution_times;
std::vector<double> EvalLog::qcreation_execution_times;
std::vector<double> EvalLog::copy_execution_times;
std::vector<double> EvalLog::step_execution_times;
std::vector<double> EvalLog::free_execution_times;
std::vector<double> EvalLog::norm_execution_times;
std::vector<double> EvalLog::vcreation_execution_times;
std::vector<double> EvalLog::init_bounds_execution_times;

std::vector<double> EvalLog::backup_execution_times;

std::vector<double> EvalLog::python_communication_times;
std::vector<int> EvalLog::python_interactions;

std::vector<std::vector<double>> EvalLog::uncertainty_values;
...
void Logger::InitRound(State* state) {
	...
	if (state) {
		state_ = state;
		std::cout << "[POMDPLogger::InitRound] Created start state." << endl;
		// Print initial state
		if (!Globals::config.silence && out_) {
...
bool Logger::SummarizeStep(int step, int round, bool terminal, ACT_TYPE action,
	...
	*out_ << "-----------------------------------Round " << round << " Step "
	...
	if (world_type_ == "pomdp")
		...
		if (reward_ > model_->GetMaxReward()) { //invalid reward from model_->Reward
			reward_ = 0;
			std::cout << "[Logger::SummarizeStep] Reward function has not been defined in DSPOMDP model" << endl;
		}
	}
	...
	if (terminal) {
		std::cout << "[RunStep] Time for step: actual / allocated = " << step_time << " / " << EvalLog::allocated_time << endl;
		if (!Globals::config.silence && out_)
			*out_ << endl;
		step_++;
		return true;
	...
	*out_ << endl;
	...
	std::cout << "[main] Time for step: actual / allocated = " << step_time << " / " << EvalLog::allocated_time << endl;
	...
	std::cout << "[main] Time per move set to " << Globals::config.time_per_move << endl;
	std::cout << "[main] Plan time ratio set to " << EvalLog::plan_time_ratio << endl;

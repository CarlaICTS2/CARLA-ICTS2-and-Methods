...
/* =============================================================================
 * DSPOMDP class: Interface for a deterministic simulative model for POMDP.
 * =============================================================================*/
class DSPOMDP {
public:
	DSPOMDP();
	 ...
	 */
	virtual bool Step(State& state, double random_num, ACT_TYPE action, double& reward, OBS_TYPE& obs) const = 0;
	 ...
	 * @param obs        Observation received after taking action
	 */
	virtual bool ImportanceSamplingStep(State& state, double random_num, ACT_TYPE action, double& reward, OBS_TYPE& obs) const;
	 ...
	 */
	virtual ScenarioUpperBound* CreateScenarioUpperBound(
		std::string name = "DEFAULT", std::string particle_bound_name = "DEFAULT"
	) const;
	 ...
	 */
	virtual ScenarioLowerBound* CreateScenarioLowerBound(
		std::string bound_name = "DEFAULT", std::string particle_bound_name = "DEFAULT"
	) const;
	 ...
	 * @param out    The destination stream
	 */
	virtual void PrintBelief(
		int id, int depth, double lower_bound, double upper_bound, std::vector<State*> particles
	) const = 0;
	 ...
	 */
	 virtual void PrintParticles(const std::vector<State*> particles) const = 0;

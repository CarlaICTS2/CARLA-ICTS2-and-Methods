...
class HyREALA2C_AUTOBOTS(RLAgent):
    def __init__(self, world, carla_map, scenario, conn=None, eval_mode=False):
        super(HyREALA2C_AUTOBOTS, self).__init__(world, carla_map, scenario)
        self.conn = conn
        ...
        if not self.eval_mode and not conn is None:
            ...
            print(m)  # RESET
        self.ped_pred = AutoBotWrapperNew(path="./ped_path_predictor/saved_models/60_80/autobots.pth")
    ...
    def get_reward(self, action):
        ...
        if speed > 1.0:
            ...
            _, goal, hit, nearmiss, terminal = super(HyREALA2C_AUTOBOTS, self).get_reward(action)
    ...
    def get_reward_despot(self, action):
        base_reward, goal, hit, nearmiss, terminal = super(HyREALA2C_AUTOBOTS, self).get_reward(action)
        reward = 0
        if goal:
            reward += 1.0
        return reward, goal, hit, nearmiss, terminal
